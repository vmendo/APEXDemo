...
echo ""
echo -e "${BLUE}📁 Moving to the project directory: /home/opc/APEXDemo/my_projects/demo ${NC}"
echo ""
cd /home/opc/APEXDemo/my_projects/demo

# Check and download artifact if not present
if [ ! -f "artifact/apex_demo-1.0.zip" ]; then
  echo -e "${BLUE}📦 Artifact apex_demo-1.0.zip not found. Downloading from GitHub release...${NC}"
  cd artifact
  gh release download v1.1 --pattern "apex_demo-1.0.zip" --dir .
  cd ..
else
  echo -e "${GREEN}✅ Artifact apex_demo-1.0.zip already exists. Skipping download.${NC}"
fi
...
echo -e "${RED}project deploy -file artifact/apex_demo-1.0.zip -verbose${NC}"
echo ""
...
sql -name apex_pro <<EOF
SET SCAN OFF;
project deploy -file artifact/apex_demo-1.0.zip -verbose
exit
EOF
